...
static JSBool
wrapSqr(JSContext *cx, JSObject *, uintN argc, jsval *argv, jsval *)
{
  if(argc!=1) {
    JS_ReportError(cx,"Wrong number of arguments");
    return JS_FALSE;
  ...
  int32 i;
  if (!JS_ValueToInt32(cx, argv[0], &i)) return JS_FALSE;
  return JS_NewNumberValue(cx,sqr(i),rval);
}
...
main()
{
  ...
  JS_DefineFunction(cx, glob, "valueToNumber", valueToNumber, 2, 0);
  JS_DefineFunction(cx, glob, "nullval", nullval, 2, 0);
  std::string script;
  char c;
