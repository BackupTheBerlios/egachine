...
#define JGACHINE_SERROR(msg) do{JGACHINE_WARN(msg);return false;}while(0)
#endif
...
Socket::connect(){
  ...
  JGACHINE_CHECK(he_ptr->h_length==sizeof(haddr));
...
bool
Socket::inAvail(const Timer::TimeStamp* timeout)
{
  fd_set read_fd_set;
  FD_ZERO(&read_fd_set);
  int fd=getHandle();
...
Socket::setBlocking(bool block)
{
  ...
  JGACHINE_CHECK(oldflags!=-1);
  // Set just the flag we want to set. 
  if (!block) oldflags |= O_NONBLOCK;
  else oldflags &= ~O_NONBLOCK;
  // Store modified flag word in the descriptor. 
  JGACHINE_CHECK(fcntl (desc, F_SETFL, oldflags)==0);
#else
  JGACHINE_WARN("not yet implemented on WIN32");
#endif
}
...
{
  if (handle==-1) return;
  if (closesocket(handle)<0) JGACHINE_WARN("close failed");
  handle=-1;
}
